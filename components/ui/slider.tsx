  ...
  return (
    ...
    >
      {typeof ariaLabel === 'string' ? (
        <SliderPrimitive.Label className="sr-only">
          {ariaLabel}
      ...
      <SliderPrimitive.Control className="data-vertical:min-h-40 relative flex w-full touch-none items-center select-none data-disabled:opacity-50 data-vertical:h-full data-vertical:w-auto data-vertical:flex-col">
        ...
        {Array.from({ length: _values.length }, (_, index) => (
          <SliderPrimitive.Thumb
            ...
            key={index}
            className="border-ring ring-ring/50 relative size-3 rounded-full border bg-white transition-[color,box-shadow] after:absolute after:-inset-2 hover:ring-3 has-[input:focus-visible]:ring-3 has-[input:focus-visible]:ring-ring/60 has-[input:focus-visible]:outline-2 has-[input:focus-visible]:outline-primary has-[input:focus-visible]:outline-offset-2 active:ring-3 block shrink-0 select-none disabled:pointer-events-none disabled:opacity-50"
          />
        ))}
