...
export function SternGerlachCascadeLab({ active, command }: { active: boolean; command: ExperimentCommand | null }) {
  ...
  return <section className="workspace sg-workspace sg-cascade-workspace" aria-labelledby="sg-cascade-title">
    <aside className="control-panel">
      <div><p className="eyebrow">09 · Mesures successives</p><h1 id="sg-cascade-title">Stern–Gerlach en cascade</h1><p className="lede">Préparer, mesurer, filtrer : l’ordre des analyseurs change les résultats.</p></div>
      <div className="control-block"><p className="control-caption">Expériences à comparer</p><div className="preset-grid sg-cascade-presets">{SG_CASCADE_PRESETS.map(preset => <Button key={preset.label} variant="outline" aria-label={preset.label} aria-pressed={config.middle === preset.middle && config.angles.every((v, i) => v === preset.angles[i]) && config.filters.every((v, i) => v === preset.filters[i])} className={config.middle === preset.middle && config.angles.every((v, i) => v === preset.angles[i]) && config.filters.every((v, i) => v === preset.filters[i]) ? 'is-selected' : ''} onClick={() => change({ angles: [...preset.angles], filters: [...preset.filters], middle: preset.middle })}>{'formula' in preset ? <Formula>{preset.formula}</Formula> : preset.label}</Button>)}</div></div>
