...
import { sampleHydrogenCloud, hydrogenCloudAtPhase, projectCloudPoint } from '@/lib/hydrogen-cloud';
  ...
  return <>
    <div className="angular-surface hydrogen-cloud" ref={frame}>
      <canvas ref={canvas} role="img" tabIndex={0} data-cloud-points={points.length} data-yaw={view.yaw} data-pitch={view.pitch}
        aria-label={`Nuage 3D de probabilité de présence : ${points.length} positions échantillonnées. Glissez ou utilisez les flèches pour tourner la vue.`}
      ...
      {['x', 'y', 'z'].map((axis, i) => { const p = projectCloudPoint({ x: i === 0 ? 1 : 0, y: i === 1 ? 1 : 0, z: i === 2 ? 1 : 0 }, view.yaw, view.pitch); return <span className="surface-axis" key={axis} aria-hidden="true" style={{ left: size.width / 2 + radius * p.x, top: size.height / 2 + radius * p.y }}><Formula>{`$${axis}$`}</Formula></span>; })}
    </div>
    ...
    <p className="scale-note">Points échantillonnés selon <Formula>{String.raw`$|\psi(\mathbf r,t)|^2$`}</Formula> : plus ils sont nombreux dans un volume, plus la présence y est probable. Ce ne sont ni plusieurs électrons ni des trajectoires. Le point central repère le noyau.</p>
  </>;
}
