...
export function FourierLab({ active, command }: { active: boolean; command: ExperimentCommand | null }) {
  ...
  const plots = useMemo(() => (['position', 'momentum'] as const).map(space => {
    ...
    const phaseSlope = (x: number) => position ? initial.momentum + current.chirp * (x - current.center) / (2 * current.sigma ** 2)
      : -initial.center - 2 * initial.sigma ** 2 * initial.chirp * (x - initial.momentum) / (1 + initial.chirp ** 2) - x * displayTime;
    ...
    const samples = right > left ? Array.from({ length: intervals + 1 }, (_, i) => sample(left + (right - left) * i / intervals)) : [];
  ...
  return <section className="workspace fourier-workspace" aria-labelledby="fourier-title">
    <aside className="control-panel">
      ...
      <div className="control-stack">
        <QuantumParameter id="fourier-sigma" label="Largeur en position" symbol={String.raw`$\sigma_0=\Delta x(0)$`} value={config.sigma} min={FOURIER_SIGMA_MIN} max={FOURIER_SIGMA_MAX} step={.05} onChange={sigma => change({ sigma })} />
    ...
    <div className="figure-panel">
      ...
      {mode === 'evolution' ? <PlaybackControls id="fourier" clock={clock} displayControl={windowControl} finalMin={.1} finalMax={FOURIER_FINAL_TIME_MAX}
        note={<>Évolution libre exacte : <Formula>$V=0$</Formula>, <Formula>{String.raw`$\hbar=m=1$`}</Formula>. La densité en impulsion reste constante. Modifier l’état initial remet le temps à zéro.</>} /> : <div className="fourier-initial-window">{windowControl}</div>}
