...
export function FourierLab({ active, command }: { active: boolean; command: ExperimentCommand | null }) {
  ...
  return <section className="workspace fourier-workspace" aria-labelledby="fourier-title">
      ...
      <div className="equation-card fourier-initial-state"><span>État initial normalisé</span>
        {harmonic ? <>
          ...
          <div className="fourier-state-representation"><p className="control-caption">En impulsion</p>
            <Formula display>{String.raw`$\widetilde\psi(p,0)=(-i)^n\sqrt{\frac b\hbar}\,e^{-ipx_0/\hbar}h_n\!\left(\frac{b(p-p_0)}{\hbar}\right)$`}</Formula>
          </div>
        ...
        <div className="fourier-state-representation">
          ...
          <Formula display>{String.raw`$\widetilde\psi(p,0)=N_p\,e^{-\frac{\sigma_0^2(p-p_0)^2}{\hbar^2}-\frac{ipx_0}{\hbar}}$`}</Formula>
    ...
    <div className="figure-panel">
      <div className="figure-heading"><div><p className="eyebrow">Position ↔ impulsion</p><h2>Deux représentations reliées par la transformée de Fourier</h2></div><span className="figure-tag"><Formula>{String.raw`$\int |\psi|^2\,dx=\int|\widetilde\psi|^2\,dp=1$`}</Formula></span></div>
      ...
      <details className="theory-notes"><summary>Repères théoriques</summary><div className="theory-grid">
        ...
        <div><span>Évolution libre</span><Formula display>{harmonic ? String.raw`$(\Delta x)^2(t)=(n+\tfrac12)\left(b^2+\frac{\hbar^2t^2}{m^2b^2}\right)$` : !gaussian ? String.raw`$\sigma^2(t)=\sigma_0^2+\frac{\hbar^2t^2}{2m^2\sigma_0^2}$` : String.raw`$\sigma^2(t)=\sigma_0^2+\frac{\hbar^2t^2}{4m^2\sigma_0^2}$`}</Formula><Formula display>{String.raw`$\widetilde\psi(p,t)=\widetilde\psi(p,0)\,e^{-ip^2t/(2m\hbar)}$`}</Formula><p>La position moyenne suit <Formula>{harmonic ? String.raw`$\langle x\rangle(t)=\langle x\rangle(0)+\frac{\langle p\rangle(0)t}{m}$` : String.raw`$\langle x\rangle(t)=x_0+\frac{p_0t}{m}$`}</Formula>. La distribution en impulsion est conservée ; seule sa phase évolue. {harmonic ? 'Les courbes et les moments sont calculés analytiquement après retrait du piège. Le paquet s’élargit en position ; il ne reste pas stationnaire comme dans un potentiel harmonique.' : gaussian ? 'Le calcul est analytique sur tout l’espace, sans parois aux bords du graphe.' : 'Les moments sont analytiques. La courbe en position est obtenue par inversion numérique de Fourier sur un domaine élargi, indépendant de la fenêtre affichée. La forme du paquet change pendant son expansion.'}</p></div>
      </div></details>
    </div>
  </section>;
}
