    ...
    const registration = context.registerTool(
      {
        ...
        description:
          `Configure les neuf laboratoires. fourier : fourierShape (gaussian/exponential/lorentzian/oscillator), oscillator propose les états propres n=0 à 5, sélectionnés par fourierNumber (entier, défaut 1, implique oscillator) ; fourierSigma est b/ell ; évolution libre après retrait du piège, fourierSigma (0.2 à 5), fourierCenter (-400 à 400, x0/ell) et fourierMomentum (-100 à 100, p0 ell/hbar), fourierView (density/complex). Paires normalisées en SI, masse de l’électron : sigma, center et fourierWindow en multiples de ell ; momentum et fourierMomentumWindow en multiples de hbar/ell ; ell=1 nm fixe : ces réglages et les axes sont sans dimension ; time et finalTime en fs. mode stationary/evolution ; time de 0 à 20, finalTime de 0.1 à 20, playbackSpeed de 0.25 à 4. fourierWindow (5 à 400, défaut 35) est la demi-largeur de fenêtre x, fixe pendant l’évolution et les changements de sigma. fourierPositionYMax et fourierMomentumYMax (0.05 à 100) fixent indépendamment les limites verticales dans la représentation sélectionnée : [0,max] pour density et [-max,max] pour complex. Pas de scale. stern-gerlach : sgSetup=single (défaut) accepte sgJ, sgGradient, sgVelocity, sgLength, sgDistance, sgAngle, sgG, sgMass, sgBeam et sgModel. Dans ce montage, time et finalTime sont en ms (time de 0 à 20, finalTime de 1 à 20), scale de 0.5 à 4 règle la taille des impacts. sgBeam polarisé sélectionne j=1/2 et le modèle quantique ; sinon j différent de 1/2 ou sgModel=classical impose mixed. sgSetup=cascade : spin 1/2 quantique uniquement, sgBeam, sgCascadeAngles=[A,B,C] en degrés, sgCascadeFilters=[A,B] (plus/minus/both), sgCascadeMiddle (false retire B). Un réglage sgCascade implique cascade. En cascade, time est en unités d’animation de 0 à 20, finalTime de 4 à 20 ; pas de paramètres de géométrie, de gradient ni de sgAngle. Pas de mode ni de preset pour Stern–Gerlach. scattering : potential, height, width, momentum, sigma, progress. double-well : barrier, separation, preset left/right, time (phase ΔE t/ℏ). rotor : angular (ℓ, défaut 1), magnetic (m, défaut 0), inertia (I/I0, défaut 1), resolution (maillage 3D, ${ROTOR_RESOLUTION_MIN} à ${ROTOR_RESOLUTION_MAX} par pas de ${ROTOR_RESOLUTION_STEP}, défaut 128). hydrogen : principal (n, défaut 1), angular (ℓ, défaut 0), magnetic (m, défaut 0), basis (complex/real), atomicView (slice/radial/cloud ; cloud = nuage de probabilité 3D orientable), plane (xz/xy/yz/oblique). Respecter |m|≤ℓ<n pour hydrogen. rotor et hydrogen acceptent mode stationary/evolution ; en évolution, choisir un preset rotor-polar/rotor-rotation ou hydrogen-breathing/hydrogen-dipole/hydrogen-rotation et time (phase ΔE t/ℏ de 0 à 20π). Ils n’utilisent pas quantumNumber. spin : spinTheta et spinPhi en degrés, spinField (x/y/z/tilted), spinMeasure (x/y/z), spinOmega (Ω/Ω0), time (Ω0t, de 0 à 20π), preset spin-x-plus/minus, spin-y-plus/minus ou spin-z-plus/minus. Les angles explicites priment sur le preset. Tous les laboratoires acceptent playbackSpeed et finalTime ; ce dernier utilise la même unité interne que time, pas t/T. scale est accepté sauf pour fourier et rotor, qui utilise resolution à la place. La lecture reste en pause.`,
        inputSchema: {
          type: 'object',
          properties: {
            lab: { type: 'string', enum: ['fourier', 'scattering', 'well', 'oscillator', 'double-well', 'rotor', 'hydrogen', 'spin', 'stern-gerlach'] },
            fourierShape: { type: 'string', enum: ['gaussian', 'exponential', 'lorentzian', 'oscillator'], description: 'Forme de l’amplitude initiale en position. Gaussian par défaut ; oscillator propose les états propres n=0 à 5.' },
            fourierNumber: { type: 'integer', minimum: 0, maximum: 5, description: 'Nombre quantique n de l’état propre d’oscillateur. Défaut 1. Implique fourierShape=oscillator.' },
            fourierSigma: { type: 'number', minimum: FOURIER_SIGMA_MIN, maximum: FOURIER_SIGMA_MAX, description: 'Écart-type initial sigma0/ell, ou longueur d’oscillateur b/ell pour oscillator, ell=1 nm fixe.' },
            fourierCenter: { type: 'number', minimum: -FOURIER_CENTER_LIMIT, maximum: FOURIER_CENTER_LIMIT, description: 'Translation x0/ell (position moyenne), ell=1 nm fixe.' },
            fourierMomentum: { type: 'number', minimum: -FOURIER_MOMENTUM_LIMIT, maximum: FOURIER_MOMENTUM_LIMIT, description: 'Translation p0 ell/hbar (impulsion moyenne), ell=1 nm fixe.' },
            fourierView: { type: 'string', enum: ['density', 'complex'] },
            fourierWindow: { type: 'number', minimum: 5, maximum: 400, description: 'Demi-largeur sans dimension de la fenêtre x/ell ; jamais ajustée automatiquement.' },
            fourierPositionYMax: { type: 'number', minimum: .05, maximum: 100, description: 'Limite verticale sans dimension du graphe en position, dans la représentation sélectionnée (density ou complex).' },
            fourierMomentumYMax: { type: 'number', minimum: .05, maximum: 100, description: 'Limite verticale sans dimension du graphe en impulsion, dans la représentation sélectionnée (density ou complex).' },
            fourierMomentumWindow: { type: 'number', minimum: 1, maximum: 100, description: 'Demi-largeur sans dimension de la fenêtre p ell/hbar, défaut 40 ; indépendante de la physique et du temps.' },
            ...
            time: { type: 'number', minimum: 0, maximum: LAB_FINAL_TIME_MAX, description: 'Fourier : temps en fs (0 à 20). Autres labos : temps réduit ou phase interne jusqu’à 20π ; Stern–Gerlach simple en ms. Diffusion : utiliser progress.' },
            ...
            finalTime: { type: 'number', minimum: LAB_FINAL_TIME_MIN, maximum: SCATTERING_FINAL_TIME_MAX, description: 'Fin de lecture : en fs pour Fourier (0.1 à 20), sinon même unité que time (phase pour double-well/rotor/hydrogen), jusqu’à 20π. Diffusion : de 1 à 120, borné selon le paquet pour limiter les effets des bords numériques.' },
            barrier: { type: 'number', minimum: 0.5, maximum: 8 },
            separation: { type: 'number', minimum: 0.8, maximum: 2.5 },
            principal: { type: 'integer', minimum: 1, maximum: 40, description: 'Hydrogène : 1 à 5, ou 10 à 40 pour les états circulaires (ell=n−1, |m|=ell). Avec hydrogen-rydberg : premier n de la paire n et n+1, de 10 à 39, défaut 20.' },
            ...
            resolution: { type: 'integer', minimum: ROTOR_RESOLUTION_MIN, maximum: ROTOR_RESOLUTION_MAX, multipleOf: ROTOR_RESOLUTION_STEP, description: 'Rotateur uniquement : subdivisions polaires du maillage 3D ; deux fois plus de subdivisions azimutales. Défaut 128. Sans effet sur la physique, le zoom ou la distribution polaire.' },
            basis: { type: 'string', enum: ['complex', 'real'] },
            atomicView: { type: 'string', enum: ['slice', 'radial', 'cloud'], description: 'Hydrogène : coupe, profil radial ou nuage de probabilité 3D orientable.' },
        ...
        async execute(input) {
          ...
          return {
            ...
            fourierShape: parsed.fourierShape ?? null,
            fourierNumber: parsed.fourierNumber ?? null,
            ...
            fourierPositionYMax: parsed.fourierPositionYMax ?? null,
            fourierMomentumYMax: parsed.fourierMomentumYMax ?? null,
          };
        },
      },
