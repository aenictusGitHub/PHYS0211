...
assert.match(fourier.html(), /Particule simulée : électron/);
assert.ok(fourier.html().includes(String.raw`\hbar^2`));
assert.match(fourier.plots()[0].xLabel, /nm/);
assert.match(fourier.plots()[1].xLabel, /10\^\{-25\}/);
...
assert.ok(fourier.html().includes(String.raw`\widetilde\psi(p,0)=N_p\,e^{-\frac{\sigma_0^2(p-p_0)^2}{\hbar^2[1-i c(0)]}-\frac{ipx_0}{\hbar}}`));
assert.ok(fourier.html().includes(String.raw`N_p=\frac{(2\sigma_0^2/\pi\hbar^2)^{1/4}}{\sqrt{1-i c(0)}}`));
...
assert.match(fourier.html(), /<output>0\.220<\/output>/);
...
near(fourier.clock().time, 2);
assert.match(fourier.html(), /<output>1\.007<\/output>/);
assert.match(fourier.html(), /<output>0\.527<\/output>/);
...
fourier.command({ fourierSigma: 1, fourierCenter: 0, fourierMomentum: 0, fourierChirp: -1, time: FOURIER_T_SCALE, finalTime: 20 });
...
fourier.command({ fourierSigma: 1, fourierMomentum: 8, fourierChirpEnabled: false, fourierWindow: 5, time: 20, finalTime: 20 });
...
fourier.command({ fourierSigma: .2, fourierCenter: .37, fourierMomentum: 0, fourierChirp: -2, fourierWindow: 400, time: .032 * FOURIER_T_SCALE, finalTime: 1 });
...
fourier.command({ fourierSigma: 1, fourierCenter: 0, fourierMomentum: 8, fourierWindow: 5, fourierMomentumWindow: 2, fourierChirpEnabled: false, time: 4 });
